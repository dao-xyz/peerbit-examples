...
test.describe("file-share churn via remote bootstrap", () => {
    test("receiver leave resets seeder count", async ({ browser, baseURL }) => {
        ...
        const fileName = `remote-churn-${Date.now()}.bin`;
        ...
        try {
            ...
            await expectSeedersAtLeast(writer, 2, 180_000);
    ...
    test("three tabs reflect churn in seeder count", async ({ browser, baseURL }) => {
        ...
        const fileName = `remote-three-tab-${Date.now()}.bin`;
        ...
        try {
            ...
            await expectSeedersAtLeast(writer, 3, 180_000);
