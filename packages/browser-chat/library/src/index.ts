...
export class Rooms extends Program implements CanOpenSubPrograms {
    ...
    async canOpen(programToOpen: Program, fromEntry: Entry<any>): Promise<boolean> {
        ...
        console.warn('Recieved an unexpected type: ' + programToOpen.constructor.name)
        return false;
    }
}


@variant(0) // for versionning purposes, we can do @variatn(1) when we create a new post type version
export class Post {
...
export class Room extends Program {
    ...
    constructor(properties?: { name: string, messages?: Documents<Post> }) {
        ...
        if (properties) {
            ...
            this.messages = properties.messages || new Documents({ canEdit: false, index: new DocumentIndex({ indexBy: 'id' }) });
