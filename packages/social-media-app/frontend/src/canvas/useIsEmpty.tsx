...
export const useCanvasStats = (canvas: Canvas | undefined) => {
    ...
    useEffect(() => {
        ...
        const checkIfEmpty = async () => {
            const iteratorElements = await canvas.elements.index.iterate({
            ...
            const iteratorReplies = await canvas.replies.index.iterate({
                query: getOwnedElementsQuery(canvas),
            });
        ...
        const listener = (
            e: CustomEvent<
                DocumentsChange<Element<ElementContent>, IndexableElement>
            >
        ) => {
