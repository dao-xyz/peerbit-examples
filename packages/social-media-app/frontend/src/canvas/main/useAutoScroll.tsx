...
export const useAutoScroll = (properties: {
    ...
    useLayoutEffect(() => {
        ...
        if (!processedReplies) {
            properties.debug && console.log("No processed replies");
            return;
        }
        if (processedReplies.length > 0) {
            if (viewIsShouldScrollToBottom) {
                ...
                const shouldScrollToBottom =
                    ...
                    scrollMode.current === "automatic";
                properties.debug &&
                    console.log("Reply change, should scroll to bottom?", {
                        shouldScrollToBottom,
                        idDiff:
                            last.reply.idString !== lastScrollToSee?.current,
                        bySelf: latestReplyRef.current.reply.publicKey.equals(
                            peer.identity.publicKey
                        ),
                        mode: scrollMode.current,
                    });
    ...
    const scrollToTop = () => {
        ...
        if (!setting.scrollUsingWindow) {
            ...
            if (repliesContainerRef.current) {
                ...
                const maxScrollY = boundingRect.top + window.scrollY - 200;

                // but we dont want to scroll down
                const currentScrollY = window.scrollY;

                const shouldScrollY = Math.min(maxScrollY, currentScrollY);

                window.scrollTo({
                    top: shouldScrollY, // 200 px extra offset
                    left: 0,
                    behavior: "instant",
                });
            } else {
