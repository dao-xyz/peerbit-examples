...
test.describe("Markdown inline giga://image", () => {
    ...
    test("renders inline giga image and hides standalone duplicate image", async ({
        page,
    }, testInfo) => {
        ...
        const detail = page.locator(`div[data-canvas-id="${replyId}"]`).first();
        await expect(detail).toBeVisible({ timeout: 60000 });
        await expect(detail.getByText(markerText).first()).toBeVisible({
            timeout: 60000,
        });

        const inlineImg = detail.getByRole("img", { name: inlineAlt }).first();
        ...
        await expect(detail.getByRole("img", { name: imgName })).toHaveCount(0);
    });
});
