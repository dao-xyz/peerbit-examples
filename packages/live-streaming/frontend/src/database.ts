...
export class VideoStream extends Program {
    ...
    constructor(sender: PublicSignKey, chunks?: Documents<Chunk>) {
        ...
        this.sender = sender;
        this.chunks = chunks || new Documents({ index: new DocumentIndex({ indexBy: 'id' }) });
    }

    async setup(): Promise<void> {
        await this.chunks.setup({
            type: Chunk, canAppend: async (entry) => {
