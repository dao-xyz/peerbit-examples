...
export const StreamOrView = () => {
    ...
    const [idArgs, setIdArgs] = useState<{ identity: PublicSignKey, node: PublicSignKey }>();
    ...
    return (
        <>
            {isStreamer !== undefined && (
                <>{isStreamer ? <Stream node={idArgs.node} identity={idArgs.identity}></Stream> : <View node={idArgs.node} identity={idArgs.identity}></View>}</>
            )}
        </>
    );
};
